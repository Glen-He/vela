...
def _run_command(*, command: tuple[str, ...], log_path: Path) -> None:
    ...
    if result.returncode != 0:
        raise ValidationError(
            f"FlexPepDock failed with exit code {result.returncode}: {log_path}"
        )
...
def _prepack_control(
    *,
    ...
    resumed = resume_completed_result(
        ...
        schema="vela.validation-prepack-result/1",
    ...
    started_at = utc_now()
    _run_command(command=command, log_path=log_path)
...
def _run_seed(
    *,
    ...
    resumed = resume_completed_result(
        ...
        schema="vela.validation-control-task-result/1",
        identity={
...
def _verify_plan(
    *, config: AppConfig, run_dir: Path, tasks: tuple[ControlTask, ...]
) -> RosettaToolInfo:
    plan_path = run_dir / PLAN_NAME
    plan = read_document(plan_path, name="qualification plan")
    if (
        plan.get("schema") != "vela.validation-qualification-plan/1"
    ...
    return verify_flexpepdock_tool(config.validation.rosetta)
    ...
    *,
    ...
    for control_id, control in controls.items():
        ...
        passed, row = analyze_control_recovery(
            control=control, tasks=control_tasks, run_dir=run_dir
        )
...
def run_qualification(*, config: AppConfig, run_dir: Path) -> QualificationOutcome:
    ...
    tool = _verify_plan(config=config, run_dir=run_dir, tasks=tasks)
    plan_path = run_dir / PLAN_NAME
    ...
    atomic_write_json(
        report_path,
        {
            "schema": "vela.validation-qualification-report/1",
