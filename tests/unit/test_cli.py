...
def test_config_check_reports_all_planned_full_surface_receptors(
    ...
    assert (
        ...
        "candidate_CA_thresholds_A=6,7,8, comparator_upper_A=10, "
        "active_CA_threshold_A=7, models_per_stratum=8" in output
    )
    ...
    assert "Currently prepared receptor-only bases: 4" in output
...
def test_preparation_status_reports_only_stage_one_data_boundary(
    ...
    assert "Receptors audited: 9" in output
    assert "Receptors prepared for Stage 2: 4" in output
